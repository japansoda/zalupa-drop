...
export const Header: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full glass-panel border-b border-white/10 bg-[#08080a]/95 max-w-full overflow-hidden">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 h-16 sm:h-22 flex items-center justify-between gap-2 sm:gap-4">
        {/* Big SVG Logo */}
        <Link 
          href="/" 
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          <button
            ...
            className="px-2 sm:px-2.5 h-8 sm:h-10 rounded-xl glass-button flex items-center justify-center font-mono font-black text-[11px] sm:text-xs text-white/80 hover:text-yellow-400 cursor-pointer transition-colors shrink-0"
          >
            {locale === 'ru' ? 'RU' : 'EN'}
          </button>
          ...
          <button
            ...
            className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl glass-button flex items-center justify-center text-white/70 hover:text-yellow-400 cursor-pointer shrink-0"
          >
          ...
          >
            <div className="flex items-center gap-1 sm:gap-1.5">
              ...
              <div className="flex flex-col text-right leading-none">
                <span className="font-mono font-black text-xs sm:text-base text-white tracking-tight">
                  {balance.toLocaleString('ru-RU')}
                </span>
                <span className="text-[8px] sm:text-[9px] font-extrabold text-yellow-400 uppercase tracking-wider hidden xs:inline">
                  DropCoin
                </span>
              </div>
            </div>
